...
class StartingVerbExtractor(BaseEstimator, TransformerMixin):
    """
    A custom transformer that checks if a sentence starts with a verb.
    ...
    def starting_verb(self, text):
        """
        Determine if the first word of the sentence is a verb.

        Args:
            text (str): A sentence to check.
    ...
    def fit(self, x, y=None):
        """
        Fit the transformer. This method is a no-op for this transformer.
    ...
    def transform(self, X):
        ...
        Args:
            X (pd.Series): A series of text data.

        Returns:
            pd.DataFrame: A DataFrame indicating whether each text starts with a verb.
        """
...
def build_model(database_filepath):
    """
    ...
    cv = GridSearchCV(pipeline, param_grid=parameters, verbose=True, cv=2)

    X, Y = load_data(database_filepath)
    #split data into train and test datasets and fit model
...
def evaluate_model(model, X_test, Y_test, database_filepath):
    ...
    #classification report for every column
    X, Y = load_data(database_filepath)
